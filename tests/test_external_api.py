...
try:
    ...
    from src.external_api import get_transaction_amount_in_rub, convert_to_rub, get_exchange_rates
...
def test_convert_eur_to_rub(mock_get):
    ...
    mock_response = Mock()
    mock_response.status_code = 200
    mock_response.json.return_value = {"success": True, "result": 8200.0}
    mock_get.return_value = mock_response
...
def test_get_transaction_amount_rub():
    ...
    transaction = {"id": 1, "operationAmount": {"amount": "1000.0", "currency": {"code": "RUB"}}}
...
def test_get_transaction_amount_usd(mock_convert):
    ...
    transaction = {"id": 2, "operationAmount": {"amount": "100.0", "currency": {"code": "USD"}}}
